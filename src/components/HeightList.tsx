...
const HeightList = () => {
  const { height } = useContext(HealthContext);

  const renderHeightItem = ({ item }: any) => (
    <View style={styles.item}>
      <Text style={styles.date}>{utils.formatTimestamp(item.timestamp)}</Text>
      <Text style={styles.value}>Altura: {item.height} Kg</Text>
    </View>
  );

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Registros de Altura</Text>
