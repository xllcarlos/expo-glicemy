...
const GlycatedHemoglobinList = () => {

  const { glycatedHemoglobin } = useContext(HealthContext) as { glycatedHemoglobin: GlycatedHemoglobin[] };

  const renderGlycatedHemoglobinItem = ({ item }: any) => (
    <View style={styles.item}>
      <Text style={styles.date}>{utils.formatTimestamp(item.timestamp)}</Text>
      <Text style={styles.value}>Hemoglobina Glicada: {item.percentage} %</Text>
    </View>
  );

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Registros de Hemoglobina Glicada</Text>
      ...
      <FlatList
        data={glycatedHemoglobin}
        keyExtractor={(item, index) => index.toString()}
        renderItem={renderGlycatedHemoglobinItem}
...
export default GlycatedHemoglobinList;